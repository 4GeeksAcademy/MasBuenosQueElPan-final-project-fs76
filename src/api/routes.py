...
@api.route('/customer_cart', methods=['POST'])
def finalize_cart():
    try:
        data = request.get_json()
        ...
        cart = CustomerCart.query.filter_by(user_id=user_id, status = 'new').first()
        if cart is None:
            return jsonify({"msg": "No hay ningún carrito activo"}), 400
        
        cart.status = 'finalized'
        db.session.add(cart)
        db.session.commit()

        return jsonify({"msg": "Carrito finalizado exitosamente!"}), 201
...
def get_customer_carts(user_id):
    try:
        carts = CustomerCart.query.filter(
            not_(CustomerCart.status.like('new')),
            CustomerCart.user_id == user_id
        ).all()
